...
class GroomingFragment : Fragment() {
    ...
    private lateinit var adapter: LayananAdapter<Grooming>
        ...
        adapter = LayananAdapter(
            ...
            onHapus = { item ->
                AlertDialog.Builder(requireContext())
                    .setTitle("Hapus Data")
                    .setMessage("Hapus grooming ${item.namaHewan}?")
                    .setPositiveButton("Hapus") { _, _ ->
                        repo.deleteGrooming(item.id)
                        adapter.updateData(repo.getAllGrooming())
                    }
                    .setNegativeButton("Batal", null).show()
            }
        )
        view.findViewById<RecyclerView>(R.id.rvGrooming).apply {
            layoutManager = LinearLayoutManager(requireContext())
            adapter = this@GroomingFragment.adapter
        }
        view.findViewById<FloatingActionButton>(R.id.fabTambahGrooming).setOnClickListener {
            showTambahDialog()
        }
    }
    ...
    private fun showTambahDialog() {
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("Simpan") { _, _ ->
                ...
                repo.addGrooming(Grooming(namaPemilik = nama, namaHewan = hewan, jenisHewan = jenis, tanggal = tanggal, jam = jam, catatan = catatan.ifBlank { "-" }))
                adapter.updateData(repo.getAllGrooming())
