...
class KonsultasiFragment : Fragment() {
    ...
    private lateinit var adapter: LayananAdapter<Konsultasi>
        ...
        adapter = LayananAdapter(
            ...
            onHapus = { item ->
                AlertDialog.Builder(requireContext())
                    .setTitle("Hapus Data")
                    .setMessage("Hapus konsultasi ${item.namaHewan}?")
                    .setPositiveButton("Hapus") { _, _ ->
                        repo.deleteKonsultasi(item.id)
                        adapter.updateData(repo.getAllKonsultasi())
                    }
                    .setNegativeButton("Batal", null).show()
            }
        )
        view.findViewById<RecyclerView>(R.id.rvKonsultasi).apply {
            layoutManager = LinearLayoutManager(requireContext())
            adapter = this@KonsultasiFragment.adapter
        }
        view.findViewById<FloatingActionButton>(R.id.fabTambahKonsultasi).setOnClickListener {
            showTambahDialog()
        }
    }
    ...
    private fun showTambahDialog() {
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("Simpan") { _, _ ->
                ...
                repo.addKonsultasi(Konsultasi(namaPemilik = nama, namaHewan = hewan, jenisHewan = jenis, tanggal = tanggal, jam = jam, catatan = catatan.ifBlank { "-" }))
                adapter.updateData(repo.getAllKonsultasi())
